...
def check_input(uin, target):
    ...
    for i in range(len(uin)):
        if uin[i] not in compared:
            compared[uin[i]] = 0
        if uin[i] == target[i]:
            print_list.append(COLOR["ENDC"]+COLOR['GREEN'])
        elif uin[i] in target and compared[uin[i]] < target_occurances[uin[i]] and exact[uin[i]] < target_occurances[uin[i]]:
            print_list.append(COLOR["ENDC"]+COLOR['YELLOW'])
        else:
            print_list.append(COLOR["ENDC"]+COLOR['WHITE'])
        print_list.append(uin[i])
        compared[uin[i]] += 1
    print_list.append(COLOR["ENDC"])
